...
import { User, Mail, Lock, Eye, EyeOff, Shield, ArrowRight, Check, X } from 'lucide-react';
...
const RegisterPage = () => {
  ...
  const [passwordStrength, setPasswordStrength] = useState(0); // 0 to 4 scale
  ...
  const checkPasswordStrength = (pass) => {
    ...
    if (/[!@#$%^&*]/.test(pass)) score++; // Special Char Check
  ...
  const handleSubmit = (e) => {
    ...
    // 3. OTP Logic:
    console.log('Register Data Validated. Sending OTP to:', formData.email);
    
    // HERE: In a real app, you would make an API call to your backend to send the email.
    // await api.sendOtp(formData.email);

    // Redirect to the Verify Email Page (passing the email state)
      ...
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-100">
        ...
        <div className="p-8">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  onChange={handleChange}
                  className={`block w-full pl-10 pr-3 py-3 border rounded-lg focus:ring-2 outline-none transition ${
                    emailError 
                ...
                {formData.email && (
                  <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                    {emailError ? <X className="h-5 w-5 text-red-500" /> : <Check className="h-5 w-5 text-green-500" />}
                  </div>
                )}
              </div>
              ...
              {formData.password && (
                <div className="mt-2 space-y-1">
                  <div className="flex gap-1 h-1.5">
                    <div className={`flex-1 rounded-full transition-all duration-300 ${passwordStrength >= 1 ? getStrengthColor() : 'bg-slate-200'}`}></div>
                    <div className={`flex-1 rounded-full transition-all duration-300 ${passwordStrength >= 2 ? getStrengthColor() : 'bg-slate-200'}`}></div>
                    <div className={`flex-1 rounded-full transition-all duration-300 ${passwordStrength >= 3 ? getStrengthColor() : 'bg-slate-200'}`}></div>
                    <div className={`flex-1 rounded-full transition-all duration-300 ${passwordStrength >= 4 ? getStrengthColor() : 'bg-slate-200'}`}></div>
                  </div>
                  <div className="flex justify-between text-xs text-slate-500">
                    <span>Strength: <span className={`font-medium ${
                      passwordStrength < 2 ? 'text-red-500' : 
                      passwordStrength < 4 ? 'text-yellow-600' : 'text-green-600'
                    }`}>{passwordFeedback || 'Too Short'}</span></span>
                    {passwordStrength < 4 && <span className="hidden sm:inline">Add symbols & numbers</span>}
                  </div>
                </div>
              )}
            </div>
            ...
            <button
              type="submit"
              disabled={!!emailError || passwordStrength < 1 || formData.password !== formData.confirmPassword}
